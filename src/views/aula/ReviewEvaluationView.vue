...
.review-page :where(a, button, input, textarea, select, [role="button"]) {
...
.review-page :where(a, button, input, textarea, select, [role="button"]):focus-visible {
  outline: 3px solid rgba(159, 25, 69, .22) !important;
  outline-offset: 3px;
}

.review-page :where(button, [role="button"], .button, .btn):not(:disabled):active {
  transform: translateY(1px) scale(.99);
}

.review-page :where(input, textarea, select) {
  font-size: max(16px, 1em);
}

.review-page :where(table tbody tr) {
  transition: background-color .18s ease;
}

.review-page :where(table tbody tr):hover {
  background-color: rgba(159, 25, 69, .025);
}

.review-page :where(.card, [class*="-card"], [class*="__card"]) {
  transition: transform .24s cubic-bezier(.2,.75,.25,1), box-shadow .24s ease, border-color .24s ease;
}

.review-page :where(.card, [class*="-card"], [class*="__card"]):hover {
  border-color: rgba(159, 25, 69, .16);
}

@media (prefers-reduced-motion: reduce) {
  .review-page *, .review-page *::before, .review-page *::after {
...
.review-page :where(input, textarea, select):focus {
