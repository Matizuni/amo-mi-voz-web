...
.inscriptions-page :where(a, button, input, textarea, select, [role="button"]) {
...
.inscriptions-page :where(a, button, input, textarea, select, [role="button"]):focus-visible {
  outline: 3px solid rgba(159, 25, 69, .22) !important;
  outline-offset: 3px;
}

.inscriptions-page :where(button, [role="button"], .button, .btn):not(:disabled):active {
  transform: translateY(1px) scale(.99);
}

.inscriptions-page :where(input, textarea, select) {
  font-size: max(16px, 1em);
}

.inscriptions-page :where(table tbody tr) {
  transition: background-color .18s ease;
}

.inscriptions-page :where(table tbody tr):hover {
  background-color: rgba(159, 25, 69, .025);
}

.inscriptions-page :where(.card, [class*="-card"], [class*="__card"]) {
  transition: transform .24s cubic-bezier(.2,.75,.25,1), box-shadow .24s ease, border-color .24s ease;
}

.inscriptions-page :where(.card, [class*="-card"], [class*="__card"]):hover {
  border-color: rgba(159, 25, 69, .16);
}

@media (prefers-reduced-motion: reduce) {
  .inscriptions-page *, .inscriptions-page *::before, .inscriptions-page *::after {
...
.inscriptions-page :where(input, textarea, select):focus {
  transform: translateY(-1px);
}
...
@media (prefers-reduced-motion: reduce) {
  .inscriptions-page,
  .inscriptions-page *,
  .inscriptions-page *::before,
  .inscriptions-page *::after {
    animation-duration: .01ms !important;
    animation-iteration-count: 1 !important;
    transition-duration: .01ms !important;
    scroll-behavior: auto !important;
