...
.teacher-review :where(a, button, input, textarea, select, [role="button"]) {
...
.teacher-review :where(a, button, input, textarea, select, [role="button"]):focus-visible {
  outline: 3px solid rgba(159, 25, 69, .22) !important;
  outline-offset: 3px;
}

.teacher-review :where(button, [role="button"], .button, .btn):not(:disabled):active {
  transform: translateY(1px) scale(.99);
}

.teacher-review :where(input, textarea, select) {
  font-size: max(16px, 1em);
}

.teacher-review :where(table tbody tr) {
  transition: background-color .18s ease;
}

.teacher-review :where(table tbody tr):hover {
  background-color: rgba(159, 25, 69, .025);
}

.teacher-review :where(.card, [class*="-card"], [class*="__card"]) {
  transition: transform .24s cubic-bezier(.2,.75,.25,1), box-shadow .24s ease, border-color .24s ease;
}

.teacher-review :where(.card, [class*="-card"], [class*="__card"]):hover {
  border-color: rgba(159, 25, 69, .16);
}

@media (prefers-reduced-motion: reduce) {
  .teacher-review *, .teacher-review *::before, .teacher-review *::after {
...
.teacher-review :where(input, textarea, select):focus {
