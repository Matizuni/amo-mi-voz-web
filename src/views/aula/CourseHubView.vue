  ...
  <section class="course-hub">
    ...
    <header class="course-cover">
      ...
      <div class="course-cover__main">
        ...
        <p>La portada académica de tu curso. Continúa tu próxima clase, revisa el calendario y accede al contenido sin perder el contexto.</p>
    ...
    <template v-else>
        ...
        <main class="overview-main">
          ...
          <section class="panel panel--next">
            ...
            <article
                v-if="nextLesson"
                class="next-class"
                :class="{ 'next-class--with-cover': nextLessonCoverUrl }"
                :style="nextLessonCoverStyle"
              >
                <div
                  v-if="nextLessonCoverUrl"
                  class="next-class__wallpaper"
                  aria-hidden="true"
                ></div>

                <div class="next-class__content">
                  <div class="date-block">
                    <strong>{{ nextLessonDay }}</strong>
                    <span>{{ nextLessonMonth }}</span>
                  </div>

                  <div class="next-class__copy">
                    <div class="next-class__eyebrow">
                      <small>CLASE {{ lessonNumber(nextLesson) }}</small>
                      <span v-if="nextLessonCoverUrl">PORTADA DE CLASE</span>
                    </div>
                    ...
                    <p>
                      {{
                        ...
                        'Revisa el contenido, actividades y materiales preparados para esta sesión.'
                      }}
                    </p>

                    <div class="next-class__meta">
                      <span
                        v-if="nextLessonDay !== '--' && nextLessonMonth !== 'S/F'"
                        class="next-class__date-chip"
                      >
                        {{ nextLessonDay }} {{ nextLessonMonth }}
                      </span>
                      <span>{{ nextLesson.time || 'Horario por confirmar' }}</span>
                      <span>{{ nextLesson.modality || 'Presencial' }}</span>
                      <span>{{ materialsForLesson(nextLesson.id) }} recursos</span>
                      <span>{{ assignmentsForLesson(nextLesson.id) }} actividades</span>
                    </div>
                  </div>
          ...
          <section class="panel">
            ...
            <header class="panel-heading"><div><span>ACTIVIDAD RECIENTE</span><h2>Actividad del curso</h2></div></header>
      ...
      <section v-else-if="activeTab === 'content'" class="content-browser">
        ...
        <header class="section-intro"><div><span>CONTENIDO DEL CURSO</span><h2>Aprende en una ruta clara y progresiva</h2><p>Explora las clases en orden académico y entra directamente a cada sesión, sus materiales y actividades.</p></div><RouterLink to="/aula/programa-formativo" class="secondary-action">Abrir gestor completo →</RouterLink></header>
      ...
      <section v-else class="announcements-view">
        ...
        <header class="section-intro"><div><span>COMUNICACIONES</span><h2>Anuncios del curso</h2><p>Espacio de comunicaciones del curso preparado para avisos académicos, cambios de horario y novedades del programa.</p></div><button v-if="isTeacher" class="secondary-action" type="button" disabled>+ Nuevo anuncio · próximo módulo</button></header>
...
const nextLessonDay = computed(() => {
  const raw = nextLesson.value?.date
  const parsed = dateParts(raw)
  if (parsed.day && parsed.day !== '--') return parsed.day
  const match = String(raw || '').match(/(\d{1,2})/)
  return match?.[1]?.padStart(2, '0') || '—'
})

const nextLessonMonth = computed(() => {
  const raw = nextLesson.value?.date
  const parsed = dateParts(raw)
  if (parsed.month && parsed.month !== 'S/F') return parsed.month
  ...
  return month ? month.slice(0,3).toUpperCase() : 'FECHA'
})
...
/* =========================================================
   COURSE HUB · PRO COURSE HOME v10
========================================================= */
.course-hub {
  --wine:#9f1945;
  --wine-dark:#7f1237;
...
.course-tabs button.active {
  background:var(--wine);
  color:#fff;
  box-shadow:0 9px 22px rgba(159,25,69,.18);
}
.course-tabs .active em {
...
.course-cover {
  ...
  border-color:var(--line);
  background:
    radial-gradient(circle at 88% 10%,rgba(217,169,29,.13),transparent 28%),
    linear-gradient(135deg,#fff,#faf9f5);
...
.course-cover__status {
  border-color:#eadfbd;
  background:#fffaf0;
}

/* La portada subida para la clase ahora es el wallpaper real de la tarjeta. */
.next-class {
  position:relative;
  min-height:330px;
  overflow:hidden;
...
.next-class--with-cover:before {
  ...
  background:linear-gradient(
    ...
    rgba(12,23,38,.88) 34%,
    rgba(12,23,38,.66) 68%,
    rgba(12,23,38,.48) 100%
  );
  opacity:1;
}
.next-class--with-cover:after {
  content:'';
  position:absolute;
  inset:auto 0 0;
  height:55%;
  z-index:-1;
  background:linear-gradient(0deg,rgba(8,17,29,.34),transparent);
}
.next-class__content {
  min-height:330px;
  display:grid;
...
.next-class--with-cover .date-block {
...
.next-class--with-cover .next-class__eyebrow > span {
...
.next-class--with-cover .next-class__copy small { color:#f4ca55; }
.next-class--with-cover .next-class__copy h3 {
  color:#fff;
  text-shadow:0 2px 14px rgba(0,0,0,.28);
}
.next-class--with-cover .next-class__copy p {
...
.next-class--with-cover .next-class__meta span {
...
.next-class .primary-action {
  ...
  background:var(--wine);
  color:#fff;
  box-shadow:0 12px 28px rgba(159,25,69,.24);
}
.next-class .primary-action:hover {
  background:var(--wine-dark);
  transform:translateY(-2px);
}

.panel,.content-browser,.calendar-view,.announcements-view {
  border-color:var(--line);
  border-radius:20px;
...
@media(max-width:800px) {
  .next-class__content { grid-template-columns:auto 1fr; }
  .next-class .primary-action { grid-column:1/-1;width:100%; }
}
@media(max-width:560px) {
  .next-class { min-height:390px; }
  .next-class__content {
    min-height:390px;
  ...
  .next-class--with-cover:before {
    background:linear-gradient(
      ...
      rgba(10,20,34,.96) 0%,
      rgba(10,20,34,.82) 60%,
      rgba(10,20,34,.36) 100%
    );
  }
  .next-class__copy p { -webkit-line-clamp:4; }
}


/* COURSE HUB v10.1 · fecha visible + identidad vino/dorado */
.next-class--with-cover .date-block{
  width:78px!important;min-width:78px!important;height:86px!important;
  display:flex!important;flex-direction:column;align-items:center;justify-content:center;
  border:1px solid rgba(247,214,111,.55)!important;border-radius:18px!important;
  background:linear-gradient(145deg,rgba(159,25,69,.94),rgba(93,14,46,.94))!important;
  box-shadow:0 14px 34px rgba(0,0,0,.25),inset 0 1px rgba(255,255,255,.16)!important;
  backdrop-filter:blur(14px)
}
.next-class--with-cover .date-block strong{
  color:#fff!important;font-size:1.7rem!important;line-height:1!important;font-weight:950!important
}
.next-class--with-cover .date-block span{
  color:#f7d66f!important;font-size:.7rem!important;font-weight:950!important;letter-spacing:.11em!important
}
.next-class--with-cover .next-class__eyebrow>span{
  border-color:rgba(247,214,111,.44)!important;
  background:rgba(130,83,8,.42)!important;color:#ffe89a!important
}
.next-class--with-cover .next-class__meta span{
  border-color:rgba(247,214,111,.24)!important;
  background:rgba(113,20,54,.58)!important;color:#fff!important
}
.next-class--with-cover .next-class__meta .next-class__date-chip{
  border-color:rgba(247,214,111,.52)!important;
  background:rgba(217,169,29,.25)!important;color:#ffe89a!important;font-weight:900
}
.next-class--with-cover .primary-action{
  background:linear-gradient(135deg,#a9194b,#821039)!important;
  box-shadow:0 14px 30px rgba(94,11,42,.34)!important
}
@media(max-width:560px){
  .next-class--with-cover .date-block{width:68px!important;min-width:68px!important;height:74px!important}
}



/* =========================================================
   COURSE HUB v10.2 · CONTRASTE DE TÍTULO + FECHA LEGACY
========================================================= */
.next-class--with-cover .next-class__copy h3 {
  ...
  text-shadow:
    ...
    0 7px 24px rgba(0,0,0,.55),
    0 0 1px rgba(255,255,255,.25) !important;
}

.next-class--with-cover .next-class__copy h3::after {
  content: '';
  display: block;
  width: 52px;
  height: 3px;
  ...
  box-shadow: 0 3px 10px rgba(217,169,29,.24);
}

/* Evita que estilos anteriores del módulo oscurezcan el título. */
.next-class--with-cover .next-class__copy h3,
.next-class--with-cover .next-class__copy h3 * {
  color: #fff8e8 !important;
}

/* La fecha debe dominar visualmente sobre el antiguo estado S/F. */
.next-class--with-cover .date-block strong {
