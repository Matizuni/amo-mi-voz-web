...
.submissions :where(a, button, input, textarea, select, [role="button"]) {
...
.submissions :where(a, button, input, textarea, select, [role="button"]):focus-visible {
  outline: 3px solid rgba(159, 25, 69, .22) !important;
  outline-offset: 3px;
}

.submissions :where(button, [role="button"], .button, .btn):not(:disabled):active {
  transform: translateY(1px) scale(.99);
}

.submissions :where(input, textarea, select) {
  font-size: max(16px, 1em);
}

.submissions :where(table tbody tr) {
  transition: background-color .18s ease;
}

.submissions :where(table tbody tr):hover {
  background-color: rgba(159, 25, 69, .025);
}

.submissions :where(.card, [class*="-card"], [class*="__card"]) {
  transition: transform .24s cubic-bezier(.2,.75,.25,1), box-shadow .24s ease, border-color .24s ease;
}

.submissions :where(.card, [class*="-card"], [class*="__card"]):hover {
  border-color: rgba(159, 25, 69, .16);
}

@media (prefers-reduced-motion: reduce) {
  .submissions *, .submissions *::before, .submissions *::after {
...
.submissions :where(input, textarea, select):focus {
