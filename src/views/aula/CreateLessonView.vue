...
.create-lesson :where(a, button, input, textarea, select, [role="button"]) {
...
.create-lesson :where(a, button, input, textarea, select, [role="button"]):focus-visible {
  outline: 3px solid rgba(159, 25, 69, .22) !important;
  outline-offset: 3px;
}

.create-lesson :where(button, [role="button"], .button, .btn):not(:disabled):active {
  transform: translateY(1px) scale(.99);
}

.create-lesson :where(input, textarea, select) {
  font-size: max(16px, 1em);
}

.create-lesson :where(table tbody tr) {
  transition: background-color .18s ease;
}

.create-lesson :where(table tbody tr):hover {
  background-color: rgba(159, 25, 69, .025);
}

.create-lesson :where(.card, [class*="-card"], [class*="__card"]) {
  transition: transform .24s cubic-bezier(.2,.75,.25,1), box-shadow .24s ease, border-color .24s ease;
}

.create-lesson :where(.card, [class*="-card"], [class*="__card"]):hover {
  border-color: rgba(159, 25, 69, .16);
}

@media (prefers-reduced-motion: reduce) {
  .create-lesson *, .create-lesson *::before, .create-lesson *::after {
...
.create-lesson :where(input, textarea, select):focus {
